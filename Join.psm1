...
([-JoinType] and [-Property]) defaults:
...
If the [-On] parameter contains an expression, the expression will be evaluated where `$_`, `$PSItem` and
...
> [!Note]
> The list of properties defined by the [-On] parameter will be complemented with the list of
> properties defined by the [-Equals] parameter and vice versa.

> [!Note]
> Related properties will be merged to a single property by default (see also the [-Property] parameter).

> [!Tip]
> If the [-On] and the [-Using] parameter are omitted, a side-by-side join is returned unless
> `OuterJoin` is performed where the default [-On] parameter value is * (all properties).

> [!Tip]
> if the left object is a scalar array, the [-On] parameters is used to name the scalar array.
...
If the [-Equals] parameter contains an expression, the expression will be evaluated where `$_`, `$PSItem` and
...
> defined by the -On parameter and vice versa. This means that by default value of the [-Equals] parameter
> is equal to the value supplied to the [-On] parameter.
...
> [!Note]
> The [-Equals] parameter can only be used with the [-On] parameter.

> [!Tip]
> if the right object is a scalar array, the [-Equals] parameters is used to name the scalar array.
...
> [!Note]
> The [-Using] parameter cannot be used with the [-On] parameter.
...
> [!Note]
> As apposed to the [-On] parameter, properties with the same name on both sides will not be renamed.

> [!Note]
> Related properties (with an equal value defined by the [-On] parameter) will be merged to a single item.
...
> [!Note]
> The default column name of (nameless) scalar array is `<Value>` this will show when the [-ValueName] parameter is omitted.
...
The [Foreach-Object] cmdlet iterates over the rows and the `-Join` operator concatenates the item collections
...
function Join-Object {
    ...
    [CmdletBinding(DefaultParameterSetName = 'On')][OutputType([Object[]])] param(
    ...
    begin {
                ...
                if ($Null -eq $Keys) {
                    $Keys =
                        ...
                        elseif ($Object.GetType().GetElementType() -and $Object.get_Count() -eq 0) { $Null }
                            ...
                            elseif ($1 -is [Collection[PSObject]]) { $Null }
                            elseif ($1 -is [Data.DataRow]) { ,@($1.Table.Columns.ColumnName) }
                            elseif ($1 -is [IDictionary]) { ,@($1.Get_Keys()) }
                            elseif ($1) { ,@($1.PSObject.Properties.Name) }
                        }
                }
                foreach ($Item in $Object) {
                    if ($Item -is [IDictionary]) { $Object; Break }
        ...
        function SetExpression ($Key = '*', $Keys, $Expression) {
            ...
            if ($Null -ne $Wildcard) {
                ...
                foreach ($Key in $Wildcard) {
                    if ($Null -ne $Key -and -not $Expressions.Contains($Key)) {
            ...
            process {
                ...
                if (-not $Where -or (& $Where)) {
                    ...
                    foreach ($Name in $Expressions.Get_Keys()) {
                        $Tuple =
                            if ($Expressions[$Name] -is [ScriptBlock]) {
                                $ScriptBlock = $Expressions[$Name]
                                @{ 0 = $Name.foreach($ScriptBlock)[0] }
                            }
                            else {
                                ...
                                elseif ($Key.Trim() -eq '*') {
                                    if ($LeftRight[$Name] -eq $Name) {
                                        if ($Null -ne $LeftIndex) { @{ 0 = $Left[$Name] } } else { @{ 0 = $Right[$Name] } }
                                    }
                                    elseif ($LeftRight.Contains($Name) -and $RightLeft.Contains($Name)) {
                                        @{ 0 = if ($Null -ne $LeftIndex) { $Left[$Name] };  1 = if ($Null -ne $RightIndex) { $Right[$Name] } }
                                    }
                                    elseif ($RightLeft.Contains($Name)) {
                                        if ($Left.Contains($Name)) { @{ 0 = $Left[$Name] } }
                                        elseif ($null -ne $RightIndex) { @{ 0 = $Right[$Name] } }
                                        elseif ($Right.Contains($RightLeft[$Name])) { @{ 0 = $Left[$RightLeft[$Name]] } } #45
                                    }
                                    elseif ($LeftRight.Contains($Name)) {
                                        if($Right.Contains($Name)) { @{ 0 = $Right[$Name] } }
                                        elseif ($null -ne $LeftIndex) { @{ 0 = $Left[$Name] } }
                                        elseif ($Left.Contains($LeftRight[$Name])) { @{ 0 = $Right[$LeftRight[$Name]] } } #45
                                    }
                                    elseif (-not $Right.Contains($Name)) { @{ 0 = $Left[$Name] } }
                                    elseif (-not $Left.Contains($Name))  { @{ 0 = $Right[$Name] } }
                                    ...
                                    } else { Throw [ArgumentNullException]::new("The property '$Key' doesn't exists", $Key) }
                                }
                            }
                        if ($Tuple -isnot [IDictionary] ) { $Node = $Null }
                        elseif ($Tuple.Count -eq 1) { $Node = $Tuple[0] }
                        else {
                            $Node = [Collection[PSObject]]::new()
                            if ($Tuple[0] -is [Collection[PSObject]]) { foreach ($Value in $Tuple[0]) { $Node.Add($Value) } } else { $Node.Add($Tuple[0]) }
                            if ($Tuple[1] -is [Collection[PSObject]]) { foreach ($Value in $Tuple[1]) { $Node.Add($Value) } } else { $Node.Add($Tuple[1]) }
                        }
                        if ($Null -ne $Discern -and $Node -is [Collection[PSObject]]) {
                            ...
                            for ($i = [math]::Min($Node.get_Count(), $Discern.Count); $i -gt 0; $i--) {
                                ...
                                if (-not $Rename) { $Rename = $ValueName}
        ...
        function ProcessObject {
            ...
            begin {
                ...
                $InnerRight = [HashSet[int]]::new()
                $RightIndices = [Dictionary[string, object]]::new($StringComparer)
                $LeftRight = @{}; $RightLeft = @{}; $LeftNull = [ordered]@{}; $RightNull = [ordered]@{}
                $LeftIndex = 0
                if ($RightObject -is [IDictionary]) { $RightList = @($RightObject) }
                else {
                    $RightName = if ($Equals.Count -eq 0 -and $On.Count -eq 1 -and $On[0] -isnot [ScriptBlock] -and "$($On[0])".Trim() -ne '*') { $On[0] }
                                 elseif ($Equals.Count -eq 1 -and $Equals[0] -isnot [ScriptBlock] -and "$($Equals[0])".Trim() -ne '*') { $Equals[0] } else { $ValueName }
            ...
            process {
                if (-not $AsDictionary) {
                    $LeftName = if ($On.Count -eq 1 -and $On[0] -isnot [ScriptBlock] -and "$($On[0])".Trim() -ne '*') { $On[0] } else { $ValueName }
                    $AsDictionary = { AsDictionary -ValueName $LeftName }.GetSteppablePipeline()
                    $AsDictionary.Begin($True)
                }
                $Left = if ($LeftObject -is[IDictionary]) { $LeftObject } elseif ($Null -ne $LeftObject) { $AsDictionary.Process((,$LeftObject))[0] }
                if (-not $LeftKeys) {
                    if ($Null -ne $Left) { $LeftKeys = $Left.get_Keys() } else { $LeftKeys = @() }
                    $Keys = [HashSet[string]]::new([string[]](@($LeftKeys) + @($RightKeys)), [StringComparer]::InvariantCultureIgnoreCase)
                }
                if ($Null -eq $Combine) {
                    if ($On.Count) {
                        ...
                        if ($OnWildCard) {
                            if ($Equals.Count -and -not $EqualsWildCard) { $On = $Equals }
                        ...
                        if ($Null -ne $Left) {
                            for ($i = 0; $i -lt $On.Count; $i++) {
                                if ( $On[$i] -is [ScriptBlock] ) { if ( $On[$i] -Like '*$Right*' ) { Write-Warning 'Use the -Using parameter for comparison expressions' } }
                                    ...
                                    if ($On[$i] -notin $LeftKeys) { Throw [MissingMemberException]::new("`$LeftObject", $On[$i]) }
                                ...
                                if ( $Equals[$i] -is [ScriptBlock] ) { if ( $On[$i] -Like '*$Left*' ) { Write-Warning 'Use the -Using parameter for comparison expressions' } }
                                    ...
                                    if ($Equals[$i] -notin $RightKeys) { Throw [MissingMemberException]::new("`$RightObject", $Equals[$i]) }
                        ...
                        foreach ($Right in $RightList) {
                            ...
                            foreach ($Name in $Equals) {
                                $Value =
                                    if ($Name -isnot [ScriptBlock])                                    { $Right[$Name] }
                                    elseif ($Right.get_Count() -eq 1 -and $Right.Contains($ValueName)) { $Right[$ValueName].foreach($Name) }
                                    else                                                               { $Right.foreach($Name)[0] }
                                $Key = # WET performance: https://github.com/orgs/PowerShell/discussions/19322
                                    if ( $Null -eq $Value ) { "$Esc`$Null" }
                                        ...
                                        elseif ($Value -is [IDictionary]) { "$Esc$Type{$($Value.GetEnumerator())}" }
                                        elseif ($Value -is [Array]) { "$Esc$Type($Value)" }
                                        else { "$Esc$Type$Value" }
                                    }
                                if (-Not --$Count) { break }
                                if (-not $Dictionary.ContainsKey($Key)) { $Dictionary[$Key] = [Dictionary[string, object]]::new($StringComparer) }
                                $Dictionary = $Dictionary[$Key]
                            }
                            if ($Dictionary.ContainsKey($Key)) { $Dictionary[$Key].Add($RightIndex++) }
                            else { $Dictionary[$Key] = [List[Int]]$RightIndex++ }
                        }
                    }
                    foreach ($Key in $LeftKeys) {
                        if ($Left[$Key] -isnot [Collection[PSObject]]) { $LeftNull[$Key] = $Null }
                        else { $LeftNull[$Key] = [Collection[PSObject]]( ,$Null * $Left[$Key].Count) }
                    }
                    foreach ($Key in $RightKeys) {
                        $RightNull[$Key] = if ($RightList) {
                            if ($RightList[0][$Key] -isnot [Collection[PSObject]]) { $Null }
                            else { [Collection[PSObject]]( ,$Null * $Left[$Key].Count) }
                        }
                    }
                    if ($Property) {
                        foreach ($Item in @($Property)) {
                            if ($Item -is [IDictionary]) { foreach ($Key in $Item.Get_Keys()) { SetExpression -Key $Key -Keys $Keys -Expression $Item[$Key] } }
                ...
                if ($Null -ne $Left) {
                    ...
                    $Indices =
                        if ($On.Count) {
                            ...
                            foreach ($Name in $On) {
                                $Value =
                                    if ($Name -isnot [ScriptBlock])                                  { $Left[$Name] }
                                    elseif ($Left.get_Count() -eq 1 -and $Left.Contains($ValueName)) { $Left[$ValueName].foreach($Name)[0] }
                                    else                                                             { $Left.foreach($Name)[0] }
                                $Key = # WET performance: https://github.com/orgs/PowerShell/discussions/19322
                                    if ( $Null -eq $Value ) { "$Esc`$Null" }
                                    else {
                                        $Type = if ($Strict) { "$($Value.GetType())" }
                        ...
                        elseif ($Using) {
                            if ($JoinType -eq 'Cross') { Throw [ArgumentException]::new('Invalid cross join', 'Using') }
                            for ($RightIndex = 0; $RightIndex -lt $RightList.Count; $RightIndex++) {
                                $Right = $RightList[$RightIndex]
                                if (& $Using) { $RightIndex }
                            }
                        }
                        elseif ($JoinType -eq 'Cross') { if ($RightList) { 0..($RightList.Length - 1) } else { @() } }
        ...
        $Parameters = [Dictionary[String,Object]]::new($PSBoundParameters)
        $Parameters['ValueName'] = $ValueName
        if ($Parameters.TryGetValue('OutBuffer', [ref]$Null))              { $Parameters['OutBuffer']   = 1   }
        if ($Parameters.ContainsKey('Discern') -and -not $Discern)         { $Parameters['Discern']     = @() }
        if ($JoinType -eq 'Outer' -and -not $Parameters.ContainsKey('On')) { $Parameters['On']          = '*' }
    ...
    process {
        ...
        trap { $PSCmdlet.ThrowTerminatingError($_) }
        if (!($Parameters.ContainsKey('LeftObject') -or $LeftList) -and -not $Parameters.ContainsKey('RightObject')) {
            Throw [ArgumentException]::new('A value for either the LeftObject, the pipeline, or the RightObject is required.')
        }
        if ($Pipeline -eq $False) { # Not yet streamed/processed
            if (-not $LeftList) {
                if ($Parameters.ContainsKey('LeftObject')) {
                    ...
                    if ($Parameters.ContainsKey('On') -and -not $Parameters.ContainsKey('Equal')) { $Parameters['SkipSameIndex'] = $True }
                    $LeftList = $RightObject
                }
            }
            if ($Parameters.ContainsKey('LeftObject')) { $Null = $Parameters.remove('LeftObject') }
            if (-not $Parameters.ContainsKey('RightObject')) { # Left self-join
                if ($Parameters.ContainsKey('On') -and -not $Parameters.ContainsKey('Equal')) { $Parameters['SkipSameIndex'] = $True }
...
$MetaData = [CommandMetadata]$JoinCommand
$ProxyCommand = [ProxyCommand]::Create($MetaData)
$Ast = [Language.Parser]::ParseInput($ProxyCommand, [ref]$null, [ref]$null)
$BeginOffset = $Ast.BeginBlock.Extent.StartOffset
$ParamBlock  = $Ast.Extent.Text.SubString(0, $BeginOffset)
$ScriptBlock = $Ast.Extent.Text.SubString($BeginOffset)

$Proxies =
    @{ Name = 'InnerJoin';      Alias = 'InnerJoin-Object'; Default = "JoinType = 'Inner'" }, # https://github.com/PowerShell/PowerShell/issues/25642
    @{ Name = 'LeftJoin';       Alias = 'LeftJoin-Object';  Default = "JoinType = 'Left'" },
    @{ Name = 'RightJoin';      Alias = 'RightJoin-Object'; Default = "JoinType = 'Right'" },
    @{ Name = 'FullJoin';       Alias = 'FullJoin-Object';  Default = "JoinType = 'Full'" },
    @{ Name = 'OuterJoin';      Alias = 'OuterJoin-Object'; Default = "JoinType = 'Outer'" },
    @{ Name = 'CrossJoin';      Alias = 'CrossJoin-Object'; Default = "JoinType = 'Cross'" },
...
$Function = [List[String]]'Join-Object'
$Alias    = [LIst[String]]'Join'
...
foreach ($Proxy in $Proxies) {
    ...
    $Function.Add($Proxy.Name)
    $Alias.Add($Proxy.Alias)
}
